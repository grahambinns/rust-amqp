...
fn main() {
    ...
    io::stdin().read_line(&mut amqp_url).ok().expect(
        "Unable to read URL!",
    );
    ...
    let mut channel =
        session.open_channel(1).ok().expect("Can't open channel!");
    ...
    println!(
        "Type some text here below, it will be sent into the '{}' queue. Hit Enter when done.",
        queue_name
    );
    let mut input_data = String::new();
    io::stdin().read_line(&mut input_data).ok().expect(
