...
fn consumer_function(
    ...
    println!(
        "[function] Content body(as string): {:?}",
...
fn main() {
    ...
    let mut channel = session.open_channel(1).ok().expect(
        "Error openning channel 1",
    );
